...
/** Get the actual aspect ratio (width / height) for an item */
...
/**
 * Pack items into rows so that items in each row can share equal height.
 * When the sum of aspect ratios in a row exceeds the target, start a new row.
 * This prevents a narrow portrait from forcing huge whitespace beside landscapes.
 */
  ...
  return (
    <>
      <div className="bg-white rounded-xl border-2 border-gray-200 shadow-sm overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-life-600 to-emerald-600 px-3 md:px-4 py-2 md:py-3">
        ...
        <div className="p-3 md:p-4 space-y-3">
                ...
                {rowIndices.map((idx) => {
                  ...
                  return (
                    ...
                    >
                      <Image
                        src={item.image}
                        alt={t('project0.donationResultAlt', { index: idx + 1 })}
                        width={800}
                        height={Math.round(800 / ratio)}
                        className="w-full h-auto transition-transform duration-500 group-hover:scale-[1.02]"
                      />

                      {/* ID overlay — only if donationIds exist */}
          ...
          {/* Verification Footer */}
          <div className="flex items-start gap-2.5 p-3 bg-life-50/80 rounded-lg border border-life-100/50">
            <CheckCircle2Icon className="w-4 h-4 text-life-500 flex-shrink-0 mt-0.5" />
            <p className="text-xs md:text-sm text-life-700 leading-relaxed">
