...
/** 处理中状态 */
export const PROCESSING_STATUSES: readonly DonationStatus[] = ['processing', 'fraud_check'] as const
...
/** 支付失败状态 */
export const FAILED_STATUSES: readonly DonationStatus[] = ['expired', 'declined', 'failed'] as const
...
export function canBatchEdit(status: DonationStatus): boolean {
  return status !== 'delivering' && ADMIN_STATUS_TRANSITIONS[status]?.length > 0
}
